...
def get_hotellist_page(city_url, page_count):
    ...
    if (page_count == 1):
        hotels_url = "http://www.tripadvisor.com" + city_url
    else:
        temp = city_url.split('Boston')
        city_url = temp[0] + 'oa'+ str(30*(page_count-1)) + '-Boston' + temp[1]
        hotels_url = "http://www.tripadvisor.com" + city_url


    headers = {'User-Agent': user_agent}
    response = requests.get(hotels_url, headers=headers)
    html = response.text.encode('utf-8')
    hotels_page = BeautifulSoup(html)

    #Find all hotels
    hotel_list = []
    for link in hotels_page.findAll("a",{ "class":"property_title" }):
        hotel_list.append(link.contents[0])

    #Save this page to a single HTML file
    hotel_html = ''
    with open(os.path.join(args.datadir, 'Boston-hotel-list-page' + str(page_count) + '.html'), "w") as h:
        for hotel in hotel_list:
            h.write(hotel)
            hotel_html = hotel_html + ' ' + hotel
    h.close()
...
def parse_hotellist_page(html):
    ...
    #Grab block of HTML code where the 'pages' URLs are listed
    pages = html.findAll("div",{"class":"unified pagination "})[0]

    #Check to see if the 'next' button is disabeld. If it is, then we don't want to continue
    next_page = pages.findAll("span",{"class":"nav next disabled"})

    #If the 'Next' button is not disabled, do:
    if not next_page:
        reviews = html.findAll("em")
        ratings = html.findAll("span",{"class":"rate"})
        print reviews

        for i in range(0,len(reviews)):
            temp = reviews[i].split("<em>")
            print temp
...
if __name__ == "__main__":
    ...
    #Get hotel html page & save list of hotels to an HTML document
    hotel_html = get_hotellist_page(city_url,1)

    #Parse hotel list page
    parse_hotellist_page(hotel_html)
